...
TEST(test_nodes, testNodeDistanceWeights) 
{
    ...
    // Make Elements
    MESH::element e1(1, elementTypeEnum::TRIANGLE, std::vector<MESH::node>{n1,n2,n3});
    MESH::element e2(2, elementTypeEnum::TRIANGLE, std::vector<MESH::node>{n2,n3,n4});
    std::vector<MESH::element> elements = {e1,e2};
    // Add elements to node
    n2.set_elements({1,2});

    /* Act */
    n2.calculateElementDistanceWeights(&elements);
...
TEST(test_mesh_entity, testMeshEntityFaces)
{
    ...
    /* Act */
    MESH::mesh_entity me(1, elementTypeEnum::TRIANGLE, std::vector<MESH::face>{f1,f2,f3});
